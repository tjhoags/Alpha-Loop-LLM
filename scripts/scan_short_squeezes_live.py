...
def scan_for_squeezes():
    ...
    if not results:
        ...
        for i, result in enumerate(results[:10], 1):
            ...
            print(f"   📊 Short %: {result['short_percent']}")
            print(f"   ⏱️  Days to Cover: {result['days_to_cover']}")
            print(f"   📈 5D Change: {result['price_change_5d']:+.1%}")
            print(f"   🔊 Volume: {result['volume_surge']:.1f}x average")
            ...
            if result['squeeze_score'] >= 0.7:
                ...
                print(f"   ⚠️  WATCH - Monitor for entry")
